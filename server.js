...
app.post('/api/transactions', async (req, res) => {
  ...
  try {
    ...
    console.log(
      `✅ 미수금내역 자동 생성: ${매출처코드} - ${미수금입금금액.toLocaleString()}원`,
    );
...
app.put('/api/purchase-statements/:date/:no', requireAuth, async (req, res) => {
  ...
  try {
    ...
    await new sql.Request(transaction)
      ...
      .input('적요패턴', sql.NVarChar(100), `%매입전표 ${거래일자}-${거래번호}%`)
      .query(`
        DELETE FROM 회계전표내역
        WHERE 사업장코드 = @사업장코드
          AND 작성일자 = @작성일자
...
app.get('/api/accounts-payable', async (req, res) => {
  try {
    ...
    let query = `
      ...
      FROM 미지급금내역
        LEFT JOIN 매입처
          ON 미지급금내역.매입처코드 = 매입처.매입처코드
          AND 매입처.사용구분 = 0
      WHERE 미지급금내역.사업장코드 = @사업장코드
    `;

    const request = pool.request().input('사업장코드', sql.VarChar(2), 사업장코드);

    if (매입처코드) {
      query += ` AND 미지급금내역.매입처코드 = @매입처코드`;
      request.input('매입처코드', sql.VarChar(8), 매입처코드);
    }

    if (시작일자 && 종료일자) {
      query += ` AND 미지급금내역.미지급금지급일자 BETWEEN @시작일자 AND @종료일자`;
      request.input('시작일자', sql.VarChar(8), 시작일자);
      request.input('종료일자', sql.VarChar(8), 종료일자);
    }

    query += ` ORDER BY 미지급금내역.미지급금지급일자 DESC, 미지급금내역.미지급금지급시간 DESC`;
