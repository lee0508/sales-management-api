...
async function openOrderModal() {
  try {
    ...
    // 매입처는 검색 모달을 통해 선택하므로 loadSuppliersForOrder() 호출 제거
    // 선택된 매입처 정보 초기화
...
/**
 * 매입처 목록 로드 (발주용)
 * 주석: HTML에서 매입처는 검색 모달을 통해 선택하므로 이 함수는 사용하지 않음
 */
...
function openSupplierSearchModal() {
  document.getElementById('supplierSearchModal').style.display = 'flex';
  // 검색 입력란 초기화
  document.getElementById('supplierSearchInput').value = '';
  // 테이블 초기화
  document.getElementById('supplierSearchTableBody').innerHTML = `
    <tr>
...
async function searchSuppliers() {
  try {
    ...
    // API 호출
    const result = await apiCall(`/suppliers?search=${encodeURIComponent(searchText)}`);
    ...
    tbody.innerHTML = result.data.map(supplier => `
      <tr style="border-bottom: 1px solid #e5e7eb;">
        ...
        <td style="padding: 12px; text-align: center;">
          <button onclick='selectSupplier("${supplier.매입처코드}", "${supplier.매입처명 || ''}")' style="
            padding: 6px 16px;
            background: #007bff;
            color: white;
            border: none;
...
function selectSupplier(code, name) {
  // 선택된 매입처 정보를 입력 필드에 설정
  document.getElementById('selectedSupplierCode').value = code;
  document.getElementById('selectedSupplierName').value = `${name} (${code})`;
  ...
  if (infoDiv && displaySpan) {
    displaySpan.textContent = `${name} (${code})`;
