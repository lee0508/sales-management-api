...
**처리 내용:**
- 거래일자/입출고일자가 1900년 이전 또는 2100년 이후인 레코드를 `사용구분=9`로 처리
- 단가=0 데이터는 **수정하지 않음** (유효한 데이터로 간주)

**Note**: `사용구분=9`는 시스템 전체에서 삭제/비활성화를 의미합니다.

## 기존 데이터 정리 결과

**전체 레코드 수**: 520,678건 → 520,676건 (2건 사용구분=9 처리)

### ✅ 자동 수정 완료
- 입출고일자 오류: 8건 → 0건
- 거래일자 오류: 8건 → 0건
- 매입처코드 누락 (실제 거래): 2건 → 0건 (사용구분=9 처리 완료)
...
- 매입처코드 누락 (단가=0): 11건
  - 입고수량=0, 입고단가=0인 테스트 데이터
  - 추후 수동 확인 필요 시 처리 가능

**수정 완료된 2건 (사용구분=9 처리):**
```
거래일자: 20251109, 거래번호: 2
- 자재: 01MOFS105 (입고수량: 1, 입고단가: 620,000원) → 사용구분=9
- 자재: 01CODE00161 (입고수량: 1, 입고단가: 70,000원) → 사용구분=9
```
